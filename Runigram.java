...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		int reverseRows = image[0].length - 1; 
		Color[][] newImage = new Color[image.length][image[0].length];
		for(int j = 0; j < image[0].length;j++){
			for(int i = 0; i < image.length;i++){
				int red = image[i][reverseRows].getRed ();
				int green = image[i][reverseRows].getGreen ();
				int blue = image[i][reverseRows].getBlue();
	...
	public static Color[][] flippedVertically(Color[][] image){
		int reverseRows = image.length - 1; 
		Color[][] newImage = new Color[image.length][image[0].length];
		for(int i = 0; i < image.length;i++){
			for(int j = 0; j < image[0].length;j++){
				int red = image[reverseRows][j].getRed ();
				int green = image[reverseRows][j].getGreen ();
				int blue = image[reverseRows][j].getBlue();
				 newImage[i][j] =  new Color(red, green, blue);
			}
			reverseRows --;
		}	
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		Color newBlendColor = new Color(red, green, blue);
		return newBlendColor;
	}
